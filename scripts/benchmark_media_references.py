    ...
    with tempfile.TemporaryDirectory(prefix="cloudctl-benchmark-") as directory:
        app = create_app(
            Settings(
                ...
                sqlite_path=Path(directory) / "bench.db",
        ...
        async with app.router.lifespan_context(app):
            ...
            async with database.unit_of_work() as session:
                ...
                for start in range(0, size, 500):
                    ...
                    await session.execute(
                        ...
                        [
                            dict(
                                ...
                                content_id="content",
                                revision_no=i + 1,
                                payload={"mediaAssetIds": ["target" if i < matches else "other"]},
                                payload_sha256="c" * 64,
                                created_by=USER,
                                created_at=now,
                            )
                    ...
                    await session.execute(
                        insert(ContentRevisionMediaRow),
                        [
                            dict(
                                id=f"reference-{i:08}",
                                tenant_id=TENANT,
                                ...
                                media_asset_id="target" if i < matches else "other",
                                created_at=now,
                            )
                            for i in indexes
                        ],
            ...
            async with httpx.AsyncClient(
                ...
                for iteration in range(35):
                    ...
                    response = await client.get(
                        "/api/v1/media/assets/target/references", headers=headers
                    )
...
async def main() -> None:
    result = dict(
        ...
        database="temporary file-backed SQLite",
        concurrency=1,
