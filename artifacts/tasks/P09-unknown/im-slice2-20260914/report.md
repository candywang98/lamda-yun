...
## 九、追加第四轮（2026-09-15 凌晨）：XHS 草稿弹窗死锁——专家会诊闭环

**根因链（控制器真机完整复现）**：XHS 内容任务成功后停在编辑器并留草稿 → 下次冷启动自动弹「继续编辑图文笔记吗？」恢复对话框 → 弹窗在场时首页「发布」锚点节点数=0（tab 栏不进语义树）→ 导航复位 5×BACK+重启全部失败 → `NAV_RESET_FAILED`，所有 XHS 任务永久阻塞。干净基线 12 步全绿证明流程本身无恙；成功任务后编辑器单次 BACK 可退出（无确认弹窗），死锁仅在冷启动草稿恢复路径。

**处置**：按用户指示升级专家智能体（gpt-6-astra，该阻塞第 1 次会诊）带完整证据链诊断并实现 → `agent/xhs-dialog-recovery` `ac90abb`：
- 独立 `BlockedDialogRegistry`（不与任务定位器混用）：XHS 白名单要求同窗口**三重共现**（标题「继续编辑图文笔记吗？」+ 可点击「存草稿」+ 可点击「去编辑」），只点「存草稿」（保草稿、不进编辑器、不丢数据）；缺任一要素/未知平台失败安全零点击。
- `NavigationReset` Port 纯决策层：根页判定失败时、BACK 每轮、relaunch 轮询每轮先关白名单弹窗；每次关闭后 800ms 重判；整次复位上限 3 个；`NAV_DIALOG_DISMISSED` 审计打点。抖音留空策略待真机证据。
- Android 292/0/0（+18 测试）。

**真机终验**：保留弹窗现场装新 APK → dispatch XHS 任务 → `NAV_DIALOG_DISMISSED label=存草稿` → **12 步全部 SUCCEEDED**（任务 `ad0ce1a4`）。此前必然失败的死锁场景变为常态自动恢复路径。HEAD `703a7ad` 已推送。

**登记**：营销过滤通道标定、相册遗留行清理时机两项不变；XHS 死锁待办关闭。
