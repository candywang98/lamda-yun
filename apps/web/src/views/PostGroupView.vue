...
import {
  createPostGroup,
  DEFAULT_POST_GROUP_NAME,
  deletePostGroup,
  listPostGroups,
  postsUsingGroup,
  replacePostGroupName,
  updatePostGroup,
  type PostGroup,
} from '@/data/post-groups'

const catalog = createPostCatalog()
const posts = ref<PostRecord[]>([])
const groups = ref<PostGroup[]>(listPostGroups())
...
const draft = ref('')
const pendingDelete = ref<PostGroup | null>(null)
...
async function load() {
  ...
  try {
    ...
    groups.value = listPostGroups(posts.value)
...
async function renamePosts(from: string, to: string) {
  if (from === to) return
  for (const post of postsUsingGroup(posts.value, from)) {
    const next = replacePostGroupName(post, from, to)
    if (!next) continue
    await catalog.save({ id: next.id, post: next })
  }
}
...
function openEdit(group: PostGroup) {
  dialog.value = { mode: 'edit', id: group.id, name: group.name, remark: group.remark }
}

function startInline(group: PostGroup, field: 'name' | 'remark') {
...
async function saveDialog() {
  ...
  try {
    if (dialog.value.mode === 'create') {
      createPostGroup({ name: dialog.value.name, remark: dialog.value.remark }, posts.value)
...
async function saveGroup(id: string, patch: { name?: string; remark?: string }) {
  ...
  const updated = updatePostGroup(id, patch, posts.value)
  if (patch.name && patch.name.trim() && patch.name.trim() !== previousName) {
    await renamePosts(previousName, updated.name)
  }
...
async function confirmDelete() {
  ...
  if (!group) return
  try {
    await renamePosts(group.name, DEFAULT_POST_GROUP_NAME)
    groups.value = deletePostGroup(group.id, posts.value)
    successMessage.value = `已删除「${group.name}」，相关帖子已归入${DEFAULT_POST_GROUP_NAME}`
    await load()
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : String(error)
  }
...
<template>
  <section class="group-page">
      ...
      <table>
        ...
        <tbody>
          <tr v-if="paged.length === 0">
            <td colspan="5" class="empty">{{ loading ? '加载中...' : '还没有帖子分组。点左上角「添加分组」创建一个。' }}</td>
          </tr>
    ...
    <div v-if="pendingDelete" class="mask" @click.self="pendingDelete = null">
      <div class="modal">
        <h3>删除分组</h3>
        <p>确定删除「{{ pendingDelete.name }}」吗？该分组下的帖子会归入{{ DEFAULT_POST_GROUP_NAME }}。</p>
        <div class="modal-actions">
