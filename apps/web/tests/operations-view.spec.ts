...
describe('OperationsView', () => {
  ...
  it('filters, selects and creates a confirmed mock run', async () => {
    await renderRoute('/operations/assets/assets-02')
    expect(screen.queryByText('竞品页面依据')).toBeNull()
    expect(screen.getByLabelText('素材名称')).toBeTruthy()
    expect(screen.getByLabelText('版本说明')).toBeTruthy()
    await fireEvent.click(screen.getByLabelText('选择 图片素材 · 01'))
  ...
  it('shows post image watermark form without adapter chrome', async () => {
    ...
    expect(screen.getByLabelText('水印字体')).toBeTruthy()
    expect(screen.getAllByText('历史水印').length).toBeGreaterThan(0)
    ...
    expect(screen.queryByLabelText('水印字体')).toBeNull()
  ...
  it('shows product editor watermark form aligned to competitor fields', async () => {
    await renderRoute('/operations/product-editor/product-editor-03')
    expect(screen.getByRole('heading', { name: '图片水印' })).toBeTruthy()
    expect(screen.getByLabelText('自定义文字')).toBeTruthy()
    expect(screen.getByLabelText('某鱼会员名')).toBeTruthy()
    expect(screen.getByLabelText('某鱼昵称')).toBeTruthy()
    expect(screen.getByLabelText('水印文字')).toBeTruthy()
    expect(screen.getByLabelText('文字阴影')).toBeTruthy()
    expect(screen.getByLabelText('关闭')).toBeTruthy()
    expect((screen.getByLabelText('水印文字') as HTMLInputElement).value).toBe('水印文字')
    expect((screen.getByLabelText('透明度') as HTMLInputElement).value).toBe('90')
    await fireEvent.click(screen.getByLabelText('图片水印'))
    expect(screen.getByText('上传logo')).toBeTruthy()
    expect(screen.getByLabelText('水印大小')).toBeTruthy()
    expect(screen.queryByLabelText('水印文字')).toBeNull()
    expect(screen.queryByLabelText('文字阴影')).toBeNull()
    expect(screen.queryByLabelText('水印字体')).toBeNull()
    expect(screen.getByRole('button', { name: '预览' })).toBeTruthy()
    expect(screen.getByRole('button', { name: '保存配置' })).toBeTruthy()
    expect(screen.queryByText('竞品页面依据')).toBeNull()
  ...
  it('renders product group management instead of the generic workbench', async () => {
    await renderRoute('/operations/product-management/product-management-03')
    expect(screen.getByText('商品分组管理')).toBeTruthy()
    expect(screen.getByRole('button', { name: '添加分组' })).toBeTruthy()
    expect(screen.getByText('默认分组')).toBeTruthy()
    expect(screen.getByText(/建议为每部设备或同一类别宝贝建立商品分组/)).toBeTruthy()
    expect(screen.queryByText('竞品页面依据')).toBeNull()
    expect(screen.queryByText('暂无执行适配器')).toBeNull()
  })

  it('renders post group management aligned to competitor fields', async () => {
    await renderRoute('/operations/post-management/post-management-04')
    expect(screen.getByText('帖子/笔记分组')).toBeTruthy()
    expect(screen.getByRole('button', { name: '添加分组' })).toBeTruthy()
    ...
    expect(screen.getByText('默认分组')).toBeTruthy()
    expect(screen.getByText('系统建立的分组')).toBeTruthy()
    expect(screen.getByText('46625')).toBeTruthy()
    expect(screen.getByText(/帖子分组可以帮助您更好地管理帖子/)).toBeTruthy()
    expect(screen.queryByText('竞品页面依据')).toBeNull()
    expect(screen.queryByText('暂无执行适配器')).toBeNull()
  })
  ...
  it('renders xiaohongshu authorized publish notes as the same workbench', async () => {
    await renderRoute('/operations/red-tasks/red-tasks-01')
    expect(screen.getByRole('heading', { name: '发布红薯笔记' })).toBeTruthy()
    expect(screen.getByText('发布的笔记')).toBeTruthy()
    expect(screen.getByLabelText('默认')).toBeTruthy()
    expect(screen.getByLabelText('均匀分配')).toBeTruthy()
    expect(screen.getByRole('button', { name: '添加发布' })).toBeTruthy()
    expect(screen.getAllByText('笔记分组').length).toBeGreaterThan(0)
    expect(screen.getByText(/此功能可将系统中的笔记发布到红薯/)).toBeTruthy()
    expect(screen.queryByText('竞品页面依据')).toBeNull()
    expect(screen.queryByText('暂无执行适配器')).toBeNull()
  })

  it('renders xiaohongshu nurture form instead of the blocked generic workbench', async () => {
    await renderRoute('/operations/red-tasks/red-tasks-03')
    expect(screen.getByRole('heading', { name: '红薯养号' })).toBeTruthy()
    expect(screen.getByLabelText('发现')).toBeTruthy()
    expect(screen.getByLabelText('关注')).toBeTruthy()
    expect(screen.getByLabelText('本地')).toBeTruthy()
    expect(screen.getByLabelText('翻屏次数')).toBeTruthy()
    expect(screen.getByLabelText('点击概率')).toBeTruthy()
    expect(screen.getByLabelText('评论内容')).toBeTruthy()
    expect(screen.getByText(/随机内容之间用 # 分割/)).toBeTruthy()
    expect(screen.queryByText('竞品页面依据')).toBeNull()
    expect(screen.queryByText('策略已阻断')).toBeNull()
  })
  ...
  it('persists the current view and opens a local record detail dialog', async () => {
    const rendered = await renderRoute('/operations/assets/assets-02')
    await fireEvent.update(screen.getByLabelText('筛选当前功能记录'), '图片素材')
    await fireEvent.update(screen.getByLabelText('版本说明'), '封面素材 v2')
    await fireEvent.click(screen.getByRole('button', { name: '保存配置' }))

    const saved = JSON.parse(window.localStorage.getItem('cloudctl:operation-view:assets-02') ?? '{}') as { search?: string; pageParameters?: Record<string, unknown> }
    expect(saved.search).toBe('图片素材')
    expect(saved.pageParameters).toEqual(expect.objectContaining({ assetScope: '全部内容', versionNote: '封面素材 v2', active: true }))
    expect(screen.getByRole('status').textContent).toContain('配置已保存到当前浏览器')

    rendered.unmount()
    await renderRoute('/operations/assets/assets-02')
    expect((screen.getByLabelText('筛选当前功能记录') as HTMLInputElement).value).toBe('图片素材')
    expect((screen.getByLabelText('版本说明') as HTMLInputElement).value).toBe('封面素材 v2')

    await fireEvent.click(screen.getAllByRole('button', { name: '查看' })[0]!)
    const dialog = screen.getByRole('dialog', { name: '记录详情' })
    expect(within(dialog).getByText('图片素材 · 01')).toBeTruthy()
    expect(within(dialog).getByText(/PDF 130 · #\/set\/material\/image/)).toBeTruthy()
    expect(within(dialog).getByText('封面素材 v2')).toBeTruthy()
  ...
  it('downloads asset JSON and filtered record CSV exports', async () => {
    ...
    expect(downloadedNames).toEqual(['cloudctl-assets-02-assets.json', 'cloudctl-assets-02-records.csv'])
