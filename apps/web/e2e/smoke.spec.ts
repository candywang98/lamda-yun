...
test('operation details expose PDF source, page workflow and domain fields', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('heading', { name: '图片水印' })).toBeVisible()
  await expect(page.getByLabel('自定义文字')).toBeVisible()
  await expect(page.getByLabel('水印文字')).toHaveValue('水印文字')
  await expect(page.getByLabel('透明度')).toHaveValue('90')
  await expect(page.getByLabel('文字阴影')).toBeVisible()
  await page.getByLabel('图片水印').click()
  await expect(page.getByText('上传logo')).toBeVisible()
  await expect(page.getByLabel('水印大小')).toHaveValue('30')
  await expect(page.getByLabel('水印文字')).toHaveCount(0)

  await page.goto('/operations/assets/assets-02')
  await expect(page.getByLabel('素材名称')).toBeVisible()
  await expect(page.getByLabel('版本说明')).toHaveValue('初始版本')
  ...
  await expect((await assetDownload).suggestedFilename()).toBe('cloudctl-assets-02-assets.json')
